...
WPI_TalonSRX tln_elevatorWheel { 5 }; //actuator wheel motor
...
void Robot::TestPeriodic() {
  // CoDriver
  if(joy_codriver.GetRawButton(0)){
    tln_cargoIO.Get() > 0.1 ? tln_cargoIO.Set(0.0) : tln_cargoIO.Set(1.0);
  }

  if(joy_codriver.GetRawButton(1)){
    tln_cargoIO.GetInverted() ? tln_cargoIO.SetInverted(false) : tln_cargoIO.SetInverted(true);
  }
  // Driver
  //Foward
  if(joy_driver.GetRawButtonPressed(0)){
    tln_elevatorWheel.Set(0.5);
  }
  if(joy_driver.GetRawButtonReleased(0)){
    tln_elevatorWheel.Set(0.0);
  }
  //BackFoward 
  if(joy_driver.GetRawButtonPressed(1)){
    tln_elevatorWheel.Set(-0.5);
  }
  if(joy_driver.GetRawButtonReleased(1)){
    tln_elevatorWheel.Set(0.0);
  }
}
